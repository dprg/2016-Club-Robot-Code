...
{
	unsigned long cnt;
	int8_t fail_cntr_us = 0;
	uint8_t fail_limit_us = 11;  // may need to be adjusted depending on # of bad us readings
	int8_t fail_cntr_bump = 0;
	uint8_t fail_limit_bump = 5;
	float trip_dist = 35.0;
	...
	while (1){
		...
		if ((us_bump_ena == ON) && (twp_ena == OFF) && ((usonic_flg == 0) || (usonic_flg == 3))) {        // ultrasonic used as bumper if ena
		...
		if ((us_bump_ena == OFF) && (twp_ena == ON) && ((usonic_flg == 0) || (usonic_flg == 3))) {        // temp wpt used with ultrasonic
		...
		if ((ptr_us_R->dist >= trip_dist) && (ptr_us_L->dist >= trip_dist)) {
			...
			//Serial3.println(fail_cntr_us);

		/*
		else {                                    // when ultrasonic not used as bumper - slow down approaching obstacle
			if (((ptr_us_R->dist < trip_dist) || (ptr_us_L->dist < trip_dist)) && (claw_closed == 0)){
				fail_cntr_us++;
				if (fail_cntr_us > fail_limit_us) {
					fail_cntr_us = fail_limit_us;
...
void move(ASIZE delay)
{
	...
	while (1) {
		segNav = delta_target(ptr_targ,ptr_temp_wpt, waypoint_x[wayptnum], waypoint_y[wayptnum],10.0);
		...
		if (stop_movement_flg == 0){
			if (rotate_in_place_flg == ON){	
				...
				sum_deg_heading_error = clip ((int)sum_deg_heading_error,-120,120);
				setpoint[R_MTR] = (double)slew_val + ((double) ptr_targ->deg_heading_error + 0.3 * sum_deg_heading_error);  // pi controller for rotation
				setpoint[L_MTR] = (double)slew_val - ((double) ptr_targ->deg_heading_error + 0.3 * sum_deg_heading_error);
