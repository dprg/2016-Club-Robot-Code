...
TSPoint TouchScreen::getPoint(void) {
  ...
  *portOutputRegister(xm_port) &= ~xm_pin;
   ...
   *portOutputRegister(yp_port) &= ~yp_pin;
   ...
   //return TSPoint(x, y, z);      //<---dp works for spd5408_tftpaint_fixed.ino
   

   // *** SPFD5408 change -- Begin
   // SPFD5408 change, because Y coordinate is inverted in this controller
   return TSPoint(x, 1023 - y, z);    //<--------------------------dp commented out
   //return TSPoint( y,1023 - x, z);      // <--------------dp suggested in instructable comments
   // -- End
}
