...
#define YP A3  // must be an analog pinz = (1023-(z2-z1));, use "An" notation!
...
void calibrate_TS(void) {
  ...
  //TS_MINX=p1.x-( (tempL*10)>>10);// 10 pixels du bord                      //<---dp commented and replaced
  //TS_MAXX=p1.x+( (tempL*tft.width())>>10);// 220 pixels entre points       //<---dp
...
uint16_t mapXValue(TSPoint p) {
  ...
  //uint16_t x = map(p.x, TS_MINX, TS_MAXX, 0, tft.width()); //<---dp commented
