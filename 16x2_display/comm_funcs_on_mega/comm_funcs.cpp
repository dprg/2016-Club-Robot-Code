...
void send_data_to_display(int num_of_data_val,int *data_array){
	...
	for (int j = 0; j < num_of_data_val; j++) {
		...
		Serial3.write(sign);	
	    //Serial.write(least_byte);       // send least significant 7 bits  range 0..127
	    //Serial.write(most_byte);         // send most significant bits (bits 8 thru 15)
	    //Serial.write(sign);		
	}
...
void communicate_display_data(uint8_t cmd) {
	...
	switch ((int) cmd)
	{
		case SONAR_DATA_CMD:
			num_of_data_val = 2;
			//eval_ultrasonic_sensors(ptr_us_R,ptr_us_L);
			eval_ultrasonic_sensor(ptr_us);
			data_values[0] = ptr_us->dist;
			data_values[1] = 0;                // only using 1 us sensor
			if ((ptr_us->new_read_flg == ON)){    //  && (ptr_us_L->new_read_flg == ON)){
				send_data_to_display(num_of_data_val,data_values);
				ptr_us->new_read_flg = OFF;
				//ptr_us_L->new_read_flg = OFF;
			}
		break;
		...
		case ENCODER_DATA_CMD:
			...
			//turn_blocking(CW,270,ptr_loc,ptr_spds);
			//delay(1000);
			//turn_blocking(CCW,-270,ptr_loc,ptr_spds);
			//delay(1000);
			//turn_blocking(CW,360,ptr_loc,ptr_spds);
		...
		case RUN_DATA_CMD:
			...
			if (first_run_pass_flg == 0){
				...
				//Serial3.print("scoop semaphore released"); 
				semaphore_release(&us_sem);
				//Serial3.print("us semaphore released");
				semaphore_release(&cam_sem);
				//Serial3.print("cam semaphore released");
				semaphore_release(&move_sem);
				//Serial3.print("move semaphore released");
				//semaphore_release(&vl53l0x_sem);
				//Serial3.print("vl53l0x semaphore released");
